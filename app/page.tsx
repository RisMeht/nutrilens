...
export default function Home() {
  ...
  useEffect(() => {
    ...
    open();
    return () => { cancelled = true; decodeControls.current?.stop(); decodeControls.current = null; stream.current?.getTracks().forEach(t => t.stop()); stream.current = null; if (video.current) video.current.srcObject = null; setCameraReady(false); };
  }, [cameraOn, facing]);
  ...
  useEffect(() => {
    ...
    reader.decodeFromVideoElement(video.current, scan => { if (scan && !cancelled) { cancelled = true; decodeControls.current?.stop(); decodeControls.current = null; barcodeResult(scan.getText()); } })
      .then(controls => { if (cancelled) controls.stop(); else decodeControls.current = controls; })
      .catch(() => {});
    return () => { cancelled = true; decodeControls.current?.stop(); decodeControls.current = null; };
